...
RC PaxRecordPageHandler::insert_record(const char *data, RID *rid)
{
  ...
  for (int i = 0; i < page_header_->column_num; i++) {
    // copy every colomn, index is the slot_num, i means which col
    char *dst_field_data = get_field_data(index, i);
    memcpy(dst_field_data, data + current_copy, get_field_len(i));
    current_copy += get_field_len(i);
  }

  ASSERT(current_copy == page_header_->record_real_size, "Data not copied.");
...
RC PaxRecordPageHandler::get_record(const RID &rid, Record &record)
{
  ...
  for (int i = 0; i < page_header_->column_num; i++) {
    ...
    memcpy(data + current_copy, field_data, get_field_len(i));
    current_copy += get_field_len(i);
  }

  ASSERT(current_copy == page_header_->record_real_size, "Data not copied in get record.");
...
RC PaxRecordPageHandler::get_chunk(Chunk &chunk)
{
  ...
  for (int i = 0; i < chunk.column_num(); i++) {
    ...
    Bitmap bitmap(bitmap_, page_header_->record_capacity);
    int    bitmap_idx = bitmap.next_setted_bit(0);
    while (bitmap_idx != -1) {
      char *data = get_field_data(bitmap_idx, col_id);
      // chunk.column_ptr(i)->append(data, 1);
      chunk.column_ptr(i)->append_one(data);
      bitmap_idx = bitmap.next_setted_bit(bitmap_idx + 1);
    }
  }
  return RC::SUCCESS;
}
